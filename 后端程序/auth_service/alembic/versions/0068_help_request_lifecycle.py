"""rebuild E6 help-request tables for assignment and answer lifecycle"""

import sqlalchemy as sa
from alembic import op

revision = "0068_help_request_lifecycle"
down_revision = "0067_scratch_review_idempotency"
branch_labels = None
depends_on = None


def upgrade() -> None:
    # E6 confirmed both tables are empty in the supported deployment databases.
    ...
    op.create_table(
        ...
        sa.Column("assigned_admin_user_id", sa.Integer(), sa.ForeignKey("admin_users.id", ondelete="RESTRICT"), nullable=True),
        ...
        sa.CheckConstraint("status IN ('open', 'answered', 'closed')", name="ck_help_requests_status"),
        sa.CheckConstraint("(status IN ('open', 'answered') AND closed_at IS NULL) OR (status = 'closed' AND closed_at IS NOT NULL)", name="ck_help_requests_status_matches_closed_at"),
...
def downgrade() -> None:
    raise RuntimeError("0068 is intentionally irreversible after the confirmed empty-table rebuild")
