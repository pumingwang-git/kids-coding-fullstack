...
def _validate_rubric(rubric: dict) -> dict:
    """量规写入即校验。`{}` 或缺 criteria = 这条挑战不用量规，合法。

    理由与 `_validate_rules` 同：拼错字段名不该静默生效。具体闸：
    - `max_score` 必须等于各准则**最高档** points 之和——不一致直接 400，**不自动纠正**
      （自动纠正意味着教研以为自己设了 100 分的量规，系统悄悄改成 85，谁也不会发现）。
    - 每项准则：`id` 满足 `^[a-z0-9_]{1,32}$` 且组内唯一、`label` 非空、至少两个档位。
    ...
    if not isinstance(max_score, int) or isinstance(max_score, bool):
        raise HTTPException(400, "量规满分 max_score 必须是整数。")
    seen_ids: set[str] = set()
    total_max = 0
    cleaned_criteria: list[dict] = []
    for ci, criterion in enumerate(criteria):
        if not isinstance(criterion, dict):
            raise HTTPException(400, f"第 {ci + 1} 项量规准则格式不正确。")
        cid = criterion.get("id")
        if not isinstance(cid, str) or not re.fullmatch(r"[a-z0-9_]{1,32}", cid):
            raise HTTPException(400, f"第 {ci + 1} 项量规准则的 id 无效（须为 1-32 位小写字母/数字/下划线）。")
        if cid in seen_ids:
            raise HTTPException(400, f"量规准则 id 重复：{cid}。")
        seen_ids.add(cid)
        label = criterion.get("label")
        if not isinstance(label, str) or not label.strip():
            raise HTTPException(400, f"第 {ci + 1} 项量规准则缺少名称。")
        levels = criterion.get("levels")
        if not isinstance(levels, list) or len(levels) < 2:
            raise HTTPException(400, f"量规准则「{label}」至少需要两个档位。")
        seen_values: set[int] = set()
        cleaned_levels: list[dict] = []
        for li, level in enumerate(levels):
            if not isinstance(level, dict):
                raise HTTPException(400, f"量规准则「{label}」的第 {li + 1} 个档位格式不正确。")
            value = level.get("value")
            if not isinstance(value, int) or isinstance(value, bool):
                raise HTTPException(400, f"量规准则「{label}」的档位 value 必须是整数。")
            if value in seen_values:
                raise HTTPException(400, f"量规准则「{label}」的档位 value 重复：{value}。")
            seen_values.add(value)
            level_label = level.get("label")
            if not isinstance(level_label, str) or not level_label.strip():
                raise HTTPException(400, f"量规准则「{label}」的第 {li + 1} 个档位缺少名称。")
            points = level.get("points")
            if not isinstance(points, int) or isinstance(points, bool) or points < 0:
                raise HTTPException(400, f"量规准则「{label}」的档位分值必须是非负整数。")
    ...
    if max_score != total_max:
        raise HTTPException(
            400, f"量规满分 max_score 应等于各准则最高档分值之和（{total_max}），"
