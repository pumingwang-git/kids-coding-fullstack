...
必须用这个 venv，系统 python 没装 pytest。全量必须独占运行，开跑前确认没有其他会话正在跑 pytest。实测共收集 **1061 条**，独占基线（2026-08-27，动态 RBAC 与 scope/capability 收口后实测）为 **1060 passed / 1 skipped / 0 failed / 34 分 01 秒**，改动小时只跑相关文件。
...
**当前无已知失败。** 曾长期挂账的判题竞态假红 `test_lesson_code_submit.py::test_judge_failure_refunds_the_try`（满载全量里偶发 `assert 'judging' == 'judge_failed'`）已由 `8b9ae5c` 收口，2026-08-27 全量实测零失败。**当时不是判错了，是判题还没跑完**：判题走进程内 `ThreadPoolExecutor`（见 `app/judge/runner.py` 头注），而该用例的 `poll()` 曾是**单次 GET、无重试**，等于赌判题线程比这次 GET 快——空载赌赢（单跑 3/3、整文件 20 passed），上千条用例抢同一个线程池时赌输。**修法已落地**：`poll()` 改成有界重试（轮询到终态或超时），不要用 sleep 定长等。以后再遇到同形状的偶发红，先照这条思路查。

**已收口：** 曾长期挂账的 `test_student_learning.py::test_unpublished_course_lesson_denied`（期望 `403`、实得 `404`）已于 2026-08-25 专项裁决收口：按《39》§3 判据，换个 `lesson_id` 结论会变，故未发布课包的课时属**范围闸**，`404` 且与「不存在」逐字相同才是规范行为——**是测试期望过时，不是实现有错**。变异佐证：拆掉 `video_play.py:65` 的 `course_visible` 闸门后，请求恰好落到 `403` 分支，说明旧期望写于该闸门加入之前。用例已改为断言 404 并补「发布后同一课时可播」哨兵。
